...
public class DateUtils extends PropertyEditorSupport {
    ...
    public static final String DATE_TIME_FORMAT_CN = "yyyy年MM月dd日HH时mm分ss秒";

    public static final String DATE_TIME_FORMAT_CN_THIS_YEAER = "MM-dd HH:mm:ss";

    public static final String DATE_TIME_FORMAT_CN_HOURS = "HH:mm:ss";
    ...
    public static final String GreenFormat = "EEE d MMM yyyy HH:mm:ss 'GMT'";
    /**
     * 一个日期早于一个日期区间
     */
    ...
    /**
     * 获取当前时间距离当月 最后一毫秒秒的差值
     *
     * @return
     */
    public static long getMvalueNowToEnd() {
        Calendar cal = Calendar.getInstance();
        Date today = cal.getTime();
...
//		return days;
    ...
    public static Boolean isToday(long timeStamp){
        ...
        int thatDay = calendar.get(Calendar.DAY_OF_YEAR);
        calendar.setTime(new Date());
        int nowYear = calendar.get(Calendar.YEAR);
        int nowDay = calendar.get(Calendar.DAY_OF_YEAR);
        if(thatYear == nowYear && thatDay == nowDay){
            return true;
        }
        return false;
    }
    ...
    public static String formatDaeForPushMsg(String time) {
        Date cTime = getDate(time, yyyy_MM_DD_HH_MM_SS);
        Calendar cal = Calendar.getInstance();
        cal.setTime(cTime);

        if (getDays(format(new Date(), yyyy_MM_DD_HH_MM_SS), time) == 0) {
            return "今天 " + transIntToString(cTime.getHours()) + ":" + transIntToString(cTime.getMinutes());
        } else if (getDays(format(new Date(), yyyy_MM_DD_HH_MM_SS), time) == 1) {
            return "昨天 " + transIntToString(cTime.getHours()) + ":" + transIntToString(cTime.getMinutes());
        } else if (getDays(format(new Date(), yyyy_MM_DD_HH_MM_SS), time) == 2) {
            return "前天 " + transIntToString(cTime.getHours()) + ":" + transIntToString(cTime.getMinutes());
        } else if (Calendar.getInstance().get(Calendar.YEAR) == cal.get(Calendar.YEAR)) {
            return transIntToString(cal.get(Calendar.MONTH) + 1) + "-" + transIntToString(cal.get(Calendar.DATE));
        } else {
            return cal.get(Calendar.YEAR) + "-" + transIntToString(cal.get(Calendar.MONTH) + 1) + "-" + transIntToString(cal.get(Calendar.DATE));
        }
    }

    public static String formateDateForIntegralMall(String time){
        Date cTime = getDate(time, yyyy_MM_DD_HH_MM_SS);
        Calendar cal = Calendar.getInstance();
        cal.setTime(cTime);

        if(cTime.getDay()!=new Date().getDay()){
            return transIntToString(cal.get(Calendar.MONTH) + 1) + "月" + transIntToString(cal.get(Calendar.DATE))+"日"
                    + transIntToString(cTime.getHours()) + "点" + transIntToString(cTime.getMinutes())+"分";
        }

        return transIntToString(cTime.getHours()) + ":" + transIntToString(cTime.getMinutes());

    }

    private static String transIntToString(int t){
        if(t < 10){
...
//  }
     ...
     * @return
     */
    public static String getCurrYearLastByDate(Date date){
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(date);
    ...
    public static void main(String[] args) {
        System.out.println(getCurrYearLastByDate(new Date()));
    }
...
//		}
     ...
     * @return
     */
    public static final String getTimeActivity(int day) {
        Date deliverDay =  getDeadline(new Date(),day);
        int DisparityDays = DateUtils.getBetweenDay(deliverDay, new Date());
        if (DisparityDays < 1) {
            return "当天";
        }
        if (DisparityDays >= 1 && DisparityDays < 2) {
        ...
        return DateUtils.getDate(deliverDay, MM_DD);

    }
    public static final String getTimeMMDD(Date deliverDay) {
        return DateUtils.getDate(deliverDay, MM_DD);
...
//		int month = date.getMonth();// 月份
     ...
     * @return x岁x月x天
     */
    public static String getBetweenFormatBirthdayTxtByBefore(Date date){
        Period between = Period.between(
                LocalDate.parse(getDate(date,DATE_FORMAT)),
                LocalDate.now()
        );
        StringBuffer sb = new StringBuffer();
        if(between.getYears() > 0){
            sb.append(between.getYears()).append("岁");
        }
        if(between.getMonths() > 0){
            sb.append(between.getMonths()).append("月");
        }
        if(between.getDays() > 0){
            sb.append(between.getDays()).append("天");
        }
        return sb.toString();
    }

    /**
     * 获取历史某一天距今间隔相差的几年几月几日按顺序依次存放数组中
     * @param date
     * @author liupei
     * @return [几年,几月,几日]
     * CalculateAgeHelper.calculate1(new Date(vo.getBabyBirthday()),new Date()) 两个计算出来的数据都不准
     */
    @Deprecated
    public static int[] getBetweenFormatArrayByBefore(Date date){
        Period between = Period.between(
                LocalDate.parse(getDate(date,DATE_FORMAT)),
                LocalDate.now()
        );
        ...
        birthday[0] = between.getYears();
        birthday[1] = between.getMonths();
        birthday[2] = between.getDays();
        return birthday;
    }
    public static String formateGreen(){
        Calendar cd = Calendar.getInstance();
        SimpleDateFormat sdf = new SimpleDateFormat(GreenFormat, Locale.US);
        sdf.setTimeZone(TimeZone.getTimeZone("GMT")); // 设置时区为GMT
        return sdf.format(cd.getTime());
    }

    public static String formateGreen(long time){
        Calendar cd = Calendar.getInstance();
    ...
    public static long getGreenTime(String greenTime){
        SimpleDateFormat sf = new SimpleDateFormat(GreenFormat, Locale.US);
        sf.setTimeZone(TimeZone.getTimeZone("GMT"));
        try {
            return sf.parse(StringUtils.replace(greenTime, ",", "")).getTime();
        } catch (ParseException e) {
            e.printStackTrace();
        }
        return 0;
    ...
    /**
     * <pre>
     *     字符串时间,转LocalDateTime
     * </pre>
     * @param
     * @author bowen
     * @return
     * @date  2018/7/24 下午8:43
     */
    public static LocalDateTime getLocalDateTime(String date, String dateFormat){
